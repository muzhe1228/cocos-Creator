<template>
  <div class="detailWrap container">
    <div class="detailWrap_nav">
      <nuxt-link to="/">考拉行情首页&nbsp;&nbsp;>&nbsp;&nbsp;</nuxt-link>
      <nuxt-link to="/Optional">自选&nbsp;&nbsp;>&nbsp;&nbsp;</nuxt-link>
      <nuxt-link :to="'/coin/'+ Index.currencyEnglisgName | toLowerCase" class="active">{{Index.currencyShortName}}详情</nuxt-link>
    </div>
    <div class="details">
      <div class="details_l">
        ...
        <div class="details_l_con">
          ...
          <div class="details_l_con_nav">
            <div class="nav_tab">
              <nuxt-link :to="'/coin/'+$route.params.id">核心数据</nuxt-link>
              <nuxt-link :to="'/coin/'+$route.params.id+'/market'">交易所行情</nuxt-link>
              <nuxt-link :to="'/coin/'+$route.params.id+'/trend'">市值趋势</nuxt-link>
              <!--<nuxt-link tag="li" :to="'/Optional/'+$route.params.id+'/msg'">最新公告</nuxt-link>-->
              <nuxt-link :to="'/coin/'+$route.params.id+'/history'">历史数据</nuxt-link>
              <nuxt-link :to="'/coin/'+$route.params.id+'/intro'">币种介绍</nuxt-link>
            </div>
      ...
      <div class="details_r">
        ...
        <ul class="details_r_con resetLoading"
          ...
          <li v-if="Details.guanw">
            <p>官网&nbsp;：&nbsp;<a class="Href" :href="Details.guanw"
                                 :title="`${Details.currencyName}-${Details.shortName}`" target="_blank">{{Details.guanw|urlRest}}</a>
            </p>
          </li>
        ...
        <ul class="details_r_bot">
          <li v-if="Details.blockChain">
            <a :href="Details.whitePaperEn" :title="Details.whitePaperEn" target="_blank">
              <p class="left">
          ...
          <li v-if="Details.officialCommunity">
            <a :href="Details.whitePaperEn" :title="Details.whitePaperEn" target="_blank">
              <p class="left">
  ...
  export default {
    ...
    async asyncData({params, query, store}) {
      return getCoinlaSingle(`/currency/${params.id}`, {currencyName: params.id}, store.state.Token).then((res) => {
        return getCoinlaSingle('/currency/findCurrencyIntra', {ccyId: res.data.data.ccyId}, store.state.Token).then((res1) => {
          return {
            Index: res.data.data,
