	...
	<a href="javascript:0;" class="active">${resultVo.currencyShortName}详情</a>
		...
		<div class="details">
			<div class="details_l">
				...
				<div class="details_l_con">
					<div class="details_l_con_nav">
						<div class="nav_tab">
							...
							<a href="javascript:0;" class="active">交易所行情</a> <a
				...
				<div class="market con_cont child">
						...
						<div>
							<c:forEach items="${list.items}" var="entity"
								...
								<ul class="table_cont">
									...
									<li><a
										href="/exchange/${fn:replace(exchangeName, ' ', '-')}"
										class="aClick" title="${entity.exchangeName}">
											${entity.exchangeName} </a></li>
									<li>${entity.currencyShortName}/${entity.currencyRightShortName}</li>
									<li>￥${entity.openTurnover}</li>
									<li>￥${entity.openPrice}</li>
									<li>${entity.openTurnoverProportion}%</li>
									<li><p class="follow" cerid="${entity.cerId}" id="market_${entity.cerId}">
							...
							<div class="newPage" data="${list.currentPage}">
								...
								<c:if test="${list.currentPage != 1}">
									<c:if test="${list.currentPage == 2}">
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market">上一页</a>
									</c:if>
									<c:if test="${list.currentPage > 2}">
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage-1}">上一页</a>
									</c:if>
								...
								<c:if test="${list.currentPage > 4}">
									<c:if test="${list.totalPage >= 8}">
										<c:if test="${list.currentPage+4 <= list.totalPage}">
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market">1</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage-3}">···</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage-2}">${ list.currentPage-2 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage-1}">${ list.currentPage-1 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage}">${ list.currentPage }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage+1}">${ list.currentPage+1 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage+2}">${ list.currentPage+2 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage+3}">···</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage}">${list.totalPage}</a>
										</c:if>
										<c:if test="${list.currentPage+4 > list.totalPage}">
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market">1</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage-6}">···</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage-5}">${ list.totalPage-5 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage-4}">${ list.totalPage-4 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage-3}">${ list.totalPage-3 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage-2}">${ list.totalPage-2 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage-1}">${ list.totalPage-1 }</a>
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage}">${list.totalPage}</a>
										</c:if>
									</c:if>
									<c:if test="${list.totalPage < 8}">
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market"
											class="">1</a>
										<c:forEach begin="2" end="${list.totalPage }" var="i">
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${i}"
												class="">${ i}</a>
										</c:forEach>
									</c:if>
								</c:if>
								<c:if test="${list.currentPage <= 4}">
									<c:if test="${list.totalPage >= 8}">
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market">1</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_2">2</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_3">3</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_4">4</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_5">5</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_6">6</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_7">···</a>
										<a
											href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.totalPage}">${list.totalPage}</a>
									</c:if>
									<c:if test="${list.totalPage < 8}">
										...
										<c:forEach begin="2" end="${list.totalPage }" var="i">
											<a
												href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${i}">${ i }</a>
										</c:forEach>
									</c:if>
								</c:if>
								...
								<c:if test="${list.totalPage > list.currentPage}">
									<a
										href="/coin/${fn:replace(ccyName, ' ', '-')}/market/list_${list.currentPage+1}">下一页</a>
								</c:if>
							</div>
						</c:if>
					</div>
...
<script src="/js/coin/market.js"></script>
</html>