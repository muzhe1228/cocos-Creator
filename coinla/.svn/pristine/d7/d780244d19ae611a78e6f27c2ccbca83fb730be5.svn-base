        ...
        <div class="cont_left">
          ...
          <div class="con_cont">
            <div class="con_cont_table resetLoading"
              ...
              <div>
                <ul class="table_cont" v-for="(item,index) in indexList.items" :key="index">
                  ...
                  <li>{{item.openCirculateTotalValue|numRest(binValue2)}}</li>
              ...
              <div class="newPage">
                <nuxt-link v-if="getPage($route.params.id)" :to="`/${setPage(prevPage($route.params.id))}`">上一页
                </nuxt-link>
                <nuxt-link class="aDisabled" v-else to="/">上一页</nuxt-link>
                <nuxt-link
                  v-for="item in pageList"
                  :to="`/${setPage(pageNumRest(item , $route.params.id))}`"
                  :class="{'active':item==getPage($route.params.id)}"
                  :key="item">{{item === 'prev'||item === 'next'? '···':item}}
                </nuxt-link>
                <nuxt-link v-if="getPage($route.params.id) < indexList.totalPage"
                           :to="`/${setPage(nextPage($route.params.id,indexList.totalPage))}`">下一页
                </nuxt-link>
                <nuxt-link class="aDisabled" v-else :to="`/${setPage(indexList.totalPage)}`">下一页</nuxt-link>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    async asyncData({store, params}) {
      return getIndex(store.state.Token, {pageSize: 50, page: getPage(params.id), conditionOne: 3})
        .then((res) => {
          return {
