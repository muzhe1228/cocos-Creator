  ...
  <div class="group">
      ...
      <div class="group_content">
        ...
        <div class="content">
          ...
          <div class="time" v-show="groupSuccessStatus">
            恭喜你组团成功！团队可瓜分<span>{{groupAwardsName}}</span>
          </div>
    ...
    <div class="shadow_wrap" v-show="shadow">
      ...
      <div class="open_award_number" v-show="awardsNumberToast">
        <h3>恭喜您获得</h3>
        <p class="number"><span>{{awardsName}}</span></p>
    ...
    <!--下载app弹框-->
    <div class="shadow_wrap"  v-show="downloadShadow" @click="hideFunc('downloadShadow')">
  ...
  export default {
    ...
    methods: {
      ...
      getWxCode () {
        ...
        let infoId = this.$route.query.infoId //通过此判断从哪个页面进来的
        if(infoId){
        ...
        if(tokenData){
          getCoinlaToken('/groupActivity/selectGroup', postData,  tokenData).then((res) => {
            ...
            if(res.data.code == 0){
              ...
              if(state == 0){
                ...
                //已成团
                //this.$toast({message: '已成团', duration: 1500})
                //判断是否已经拆过红包
                _this.isOpenAward()
                this.groupAwardsName = resData.awardsName
          ...
          getCoinlaSingle('/groupActivity/selectGroup', postData).then((res) => {
            ...
            if(res.data.code == 0){
              ...
              getCoinlaSingle('/groupActivity/selectGroup', {
                activityId: 2,
                infoId: this.$route.query.infoId || this.$route.params.infoId
              }).then((res) => {
                let _this = this
      ...
      isOpenAward () {
        //微信中通过判断token是否登录app
        if(this.headersData || this.wxHeadersData || this.$route.params.headersToken) {
          getCoinlaToken('/groupActivity/selectReceiveState', {
            activityId: 2,
            infoId: this.infoId
          }, this.headersData || this.wxHeadersData || this.$route.params.headersToken).then((res) => {
            if(res.data.data == 0){
              //未拆红包
      ...
      organizeGroup () {
        getCoinlaToken('/groupActivity/insertGroup', {
          activityId: 2,
          uuid: this.userUuidData || this.wxGroupData.uuid || this.$route.params.uuid,
        }, this.headersData || this.wxHeadersData || this.$route.params.headersToken).then((res) => {
          if(res.data.code == 0){
            this.isOpenInApp()
          }else{
      ...
      getMyPrize () {
        getCoinlaSingle('/groupActivity/selectMyPrize', {
          ...
          this.$toast({message: '奖励记录标识', duration: 1500})
        })
      },
      //组团结束倒计时
      ...
      //跳转到我参与的团列表页面
      groupRecord () {
        this.$router.push({name: 'groupRecord', params: {
          uuid: this.userUuidData || this.wxGroupData.uuid || this.$route.params.uuid,
          headersToken: this.headersData || this.wxHeadersData || this.$route.params.headersToken
        }
        })
      },
      //跳转到我的奖励列表页面
      groupAward (){
        this.$router.push({name: 'groupAward', params: {
          ...
          infoId: this.infoId,
          headersToken: this.headersData || this.wxHeadersData ||this
        }})
      },
      getGroupAward (){
        ...
        if(openInApp()){
          ...
          getCoinlaSingle('/share/multi', {type: 9}).then((res) => {
            if(res.data.code === 0){
              ...
              this.$toast({message: '右上角点击分享', duration: 1500})
            }
          }).catch((err) => {})
        }
      },
