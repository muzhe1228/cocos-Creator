...
@ccclass
export default class BidWindowController extends WindowController {
    ...
    public setCanBidding(bool: boolean) {
        ...
        if (bool) {
            ...
            this.leftButton.node.on(cc.Node.EventType.TOUCH_END,
                () => { cb(this.leftMultipleId) }, this);
            this.rightButton.node.on(cc.Node.EventType.TOUCH_END,
                () => { cb(this.rightMultipleId) }, this);
    ...
    private requestBidingCallback(res: any) {
        ...
        if (res.code == 0) {
            this.playAudioClip("sounds/飞筹码");
        }
    }

    private requestMultipleId (callback?: Function) {
        this.httpUtils.post(
    ...
    private requestMultipleIdCallback (res: any, callback?: Function) {
        let list = res.data;
        list.sort((a, b) => { return a.multiple - b.multiple });
    ...
    public setRound (num: number | string) {
        this.round.string = num.toString();
    }
