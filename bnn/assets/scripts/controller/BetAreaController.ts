...
export default class BetAreaController extends CardsAndResultComponent {
    ...
    // 渲染下注
    public renderBet(betPositionItem: UserBetPositionItem, chipWorldPosition: cc.Vec2, isMyself: boolean = false, chips: number[] = []) {
    ...
    private renderBetAction(chipValue: number, chipWorldPosition: cc.Vec2) {
        ...
        // step4. 局部空间下执行动作
        flyChip.runAction(cc.spawn(
            cc.moveTo(0.5, dstPosition).easing(cc.easeOut(1.0)),
            cc.fadeTo(0.15, 255)
        ));
    }
}
